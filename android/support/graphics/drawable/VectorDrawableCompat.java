...
public class VectorDrawableCompat extends VectorDrawableCommon {
  ...
  private void updateStateFromTypedArray(TypedArray paramTypedArray, XmlPullParser paramXmlPullParser) throws XmlPullParserException {
    ...
    float f = TypedArrayUtils.getNamedFloat(paramTypedArray, paramXmlPullParser, "viewportHeight", 8, vPathRenderer.mViewportHeight);
    vPathRenderer.mViewportHeight = f;
    if (vPathRenderer.mViewportWidth > 0.0F) {
      if (f > 0.0F) {
        vPathRenderer.mBaseWidth = paramTypedArray.getDimension(3, vPathRenderer.mBaseWidth);
        f = paramTypedArray.getDimension(2, vPathRenderer.mBaseHeight);
        vPathRenderer.mBaseHeight = f;
  ...
  public float getPixelSize() {
    ...
    if (vectorDrawableCompatState != null) {
      ...
      if (vPathRenderer != null) {
        ...
        if (f != 0.0F) {
          float f1 = vPathRenderer.mBaseHeight;
          if (f1 != 0.0F) {
            float f2 = vPathRenderer.mViewportHeight;
            if (f2 != 0.0F) {
              float f3 = vPathRenderer.mViewportWidth;
              if (f3 != 0.0F)
                return Math.min(f3 / f, f2 / f1); 
            } 
          } 
        } 
      } 
  ...
  private static class VGroup {
    ...
    public VGroup(VGroup param1VGroup, ArrayMap<String, Object> param1ArrayMap) {
      ...
      matrix.set(param1VGroup.mLocalMatrix);
  ...
  private static class VPathRenderer {
    ...
    private void drawPath(VectorDrawableCompat.VGroup param1VGroup, VectorDrawableCompat.VPath param1VPath, Canvas param1Canvas, int param1Int1, int param1Int2, ColorFilter param1ColorFilter) {
      ...
      if (param1VPath.isClipPath()) {
        ...
        float f = ((VectorDrawableCompat.VFullPath)param1VPath).mTrimPathStart;
        if (f != 0.0F || ((VectorDrawableCompat.VFullPath)param1VPath).mTrimPathEnd != 1.0F) {
          float f4 = ((VectorDrawableCompat.VFullPath)param1VPath).mTrimPathOffset;
          float f5 = ((VectorDrawableCompat.VFullPath)param1VPath).mTrimPathEnd;
          ...
          path.reset();
          if (f > f5) {
            this.mPathMeasure.getSegment(f, f2, path, true);
            this.mPathMeasure.getSegment(0.0F, f5, path, true);
          } else {
            this.mPathMeasure.getSegment(f, f5, path, true);
          } 
...
/* Location:              /home/dragonh/workspace/work/app/apk-reverse/dex-tools-2.1/target-dex2jar.jar!/android/support/graphics/drawable/VectorDrawableCompat.class
