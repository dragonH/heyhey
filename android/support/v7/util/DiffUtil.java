...
public class DiffUtil {
  ...
  public static DiffResult calculateDiff(Callback paramCallback, boolean paramBoolean) {
    ...
    while (!arrayList1.isEmpty()) {
      ...
      Snake snake = diffPartial(paramCallback, range.oldListStart, range.oldListEnd, range.newListStart, range.newListEnd, arrayOfInt1, arrayOfInt2, j);
      if (snake != null) {
        Range range1;
        if (snake.size > 0)
          arrayList.add(snake); 
        ...
        if (snake.reverse) {
          if (snake.removal) {
            int k = snake.x;
            i = snake.size;
            range.oldListStart = k + i + 1;
            range.newListStart = snake.y + i;
          } else {
            i = snake.x;
            int k = snake.size;
            range.oldListStart = i + k;
            range.newListStart = snake.y + k + 1;
          } 
        } else {
          i = snake.x;
          int k = snake.size;
          range.oldListStart = i + k;
          range.newListStart = snake.y + k;
        } 
  ...
  private static Snake diffPartial(Callback paramCallback, int paramInt1, int paramInt2, int paramInt3, int paramInt4, int[] paramArrayOfint1, int[] paramArrayOfint2, int paramInt5) {
    ...
    Arrays.fill(paramArrayOfint1, m, paramInt4, 0);
    Arrays.fill(paramArrayOfint2, m + j, paramInt4 + j, paramInt2);
    if (j % 2 != 0) {
      m = 1;
    } else {
      m = 0;
        ...
        if (k != 0) {
          if (k != 4 && k != 8) {
            if (k == 16) {
              ...
              stringBuilder.append(Long.toBinaryString(k));
            ...
            if (k == 4)
              param1ListUpdateCallback.onChanged(param1Int1, 1, this.mCallback.getChangePayload(j, i)); 
          } 
        } else {
      ...
      while (--param1Int2 >= 0) {
        ...
        if (k != 0) {
          if (k != 4 && k != 8) {
            ...
            if (k == 4)
              param1ListUpdateCallback.onChanged(postponedUpdate.currentPos - 1, 1, this.mCallback.getChangePayload(i, j)); 
          } 
        } else {
    ...
    private boolean findMatchingItem(int param1Int1, int param1Int2, int param1Int3, boolean param1Boolean) {
      ...
      while (param1Int3 >= 0) {
        ...
        if (param1Boolean) {
          ...
          for (param1Int2 = i - 1; param1Int2 >= n + m; param1Int2--) {
    ...
    private void findMatchingItems() {
      ...
      for (int k = this.mSnakes.size() - 1; k >= 0; k--) {
        ...
        for (j = 0; j < snake.size; j++) {
          i1 = snake.x + j;
          int i2 = snake.y + j;
          if (this.mCallback.areContentsTheSame(i1, i2)) {
            i = 1;
          } else {
            i = 2;
          } 
          this.mOldItemStatuses[i1] = i2 << 5 | i;
          this.mNewItemStatuses[i2] = i1 << 5 | i;
        } 
        i = snake.x;
        j = snake.y;
      } 
      ...
      for (int k = this.mSnakes.size(); --k >= 0; k--) {
        ...
        for (j = m - 1; j >= 0; j--) {
          ...
          if ((arrayOfInt[i + j] & 0x1F) == 2)
            param1ListUpdateCallback.onChanged(i + j, 1, this.mCallback.getChangePayload(i + j, snake.y + j)); 
        } 
        i = snake.x;
        j = snake.y;
      } 
...
/* Location:              /home/dragonh/workspace/work/app/apk-reverse/dex-tools-2.1/target-dex2jar.jar!/android/support/v7/util/DiffUtil.class
